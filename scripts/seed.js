...
/**
 * AGILE PRIME — Database Seed Script (CommonJS)
 * Usage:
...
const MONGO_URI =
  ...
  'mongodb://127.0.0.1:27017/agile-prime';
...
function sumMp(mp) {
  return (mp.steelFixer || 0) + (mp.steelFixerForemen || 0) +
         (mp.carpenter  || 0) + (mp.carpenterForemen  || 0) +
         (mp.helper     || 0) + (mp.scaffolding       || 0) +
         (mp.engineersNo|| 0);
}
...
// ─── main ────────────────────────────────────────────────────────────────────
async function seed() {
  console.log('🌱 AGILE PRIME Seed Script');
  ...
  if (FRESH) {
    ...
    await Engineer.deleteMany({
      name: { $nin: ['Eng Mohamed El Araby','Eng Mohamed El Esawy','Eng Samer El Dheby','Eng Abdelrhman Atif','Eng Omar','Eng Ahmed Ibrahim'] },
    });
    await Project.deleteMany({ name: { $nin: ['SkyBlade','Palm 01','Canal Three'] } });
  ...
  if (!adminExists) {
    ...
    await AdminUser.create({
      ...
      passwordHash: hash, role: 'superadmin', isActive: true,
      permissions: {
  ...
  // ── engineers ─────────────────────────────────────────────────────────────
  const engineerDefs = [
    { name: 'Eng Mohamed El Araby', phone: '+971500000001' },
    { name: 'Eng Mohamed El Esawy', phone: '+971500000002' },
    { name: 'Eng Samer El Dheby',   phone: '+971500000003' },
    { name: 'Eng Abdelrhman Atif',  phone: '+971500000004' },
    { name: 'Eng Omar',             phone: '+971500000005' },
    { name: 'Eng Ahmed Ibrahim',    phone: '+971500000006' },
  ];
  ...
  const projectDefs = [
    {
      name: 'SkyBlade', code: 'SKY-001', location: 'Business Bay, Dubai',
      levels: ['Basement-3','Basement-2','Basement-1','Ground Floor','Mezzanine','Podium 1','Podium 2','Podium 3','Level 1','Level 2','Level 3','Level 4','Level 5','Roof'],
    },
    {
      name: 'Palm 01', code: 'PLM-002', location: 'Palm Jebel Ali, Dubai',
      levels: ['Foundation','Ground Floor','First Floor','Roof','External Works','Landscaping'],
    },
    {
      name: 'Canal Three', code: 'CAN-003', location: 'Business Bay, Dubai',
      levels: ['Basement-2','Basement-1','Ground Floor','Mezzanine','Level 1','Level 2','Level 3','Level 4','Level 5','Level 6','Level 7','Level 8','Roof','Roof Top Amenity'],
    },
  ];
  ...
  // ── elements ──────────────────────────────────────────────────────────────
  const elements = [
    'Column','Beam','Slab','Wall','Shear Wall','Staircase','Ramp','Retaining Wall',
    'Raft Foundation','Pile Cap','Grade Beam','Drop Panel','Parapet','Planter Box',
    'Swimming Pool','Water Tank','Lift Core','Facade Panel',
  ];
  ...
  // ── activities ────────────────────────────────────────────────────────────
  const activities = [
    'Formwork','Rebar Installation','Concrete Pouring','Curing','Waterproofing',
    'Backfilling','Excavation','Scaffolding Erection','Scaffolding Dismantling',
    'Block Work','Plastering','Tiling','Painting','MEP Roughing','MEP Finishing',
    'Steel Fixing','Shuttering','De-shuttering','Inspection',
  ];
  ...
  const supplierNames = [
    'Al Habtoor Labor',
    'Gulf Manpower Solutions',
    'Drake & Scull Workforce',
    'National Labor Supply',
    'Emirates Staffing Group',
    'Transguard Manpower',
    'Golden Falcon Manpower',
    'Al Rostamani Labor',
  ];
    ...
    for (const date of DATE_RANGE) {
      ...
      for (let r = 0; r < reportsPerDay; r++) {
        ...
        for (let i = 0; i < itemCount; i++) {
          ...
          const extMp    = isExt ? buildManpower() : { steelFixer:0, steelFixerForemen:0, carpenter:0, carpenterForemen:0, helper:0, scaffolding:0, engineersNo:0 };
          const extMpTot = isExt ? sumMp(extMp) : 0;

          const companyName =
            srcType === 'Supplier'      ? pick(supplierNames)      :
            srcType === 'Subcontractor' ? pick(subcontractorNames) : '';
        ...
        const avgProg = Math.round(
          (items.reduce((s, it) => s + it.progress, 0) / items.length) * 100
        ) / 100;
